...
Purpose:
- Verify manifests on original signed images and videos in data/manifests/
...
Input Assets:
- data/manifests/images/*_signed.png (100 images, SD1.4)
- data/manifests/videos/internal/*_signed.mp4 (50 videos, SVD)
- data/manifests/videos/external/*_signed.mp4 (60 videos, Veo3.1)

Output:
- data/metrics/c2pa_validation_baseline.csv
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler('data/metrics/verify_original_manifests.log')
    ]
)

# Configuration
MANIFESTS_BASE = Path("data/manifests")
OUTPUT_CSV = Path("data/metrics/c2pa_validation_baseline.csv")
...
def collect_signed_assets() -> list:
    """
    Collect all signed assets from data/manifests/.
...
def process_assets():
    ...
    """
    assets = collect_signed_assets()

    if not assets:
        logging.error(f"No signed assets found in {MANIFESTS_BASE}")
        return

    logging.info(f"Found {len(assets)} signed assets to verify")
    ...
    logging.info("=" * 60)
    logging.info("C2PA Baseline Verification Complete")
...
def print_summary_statistics():
    ...
    try:
        ...
        logging.info("")
        logging.info("Baseline Verification Summary:")
        logging.info(f"  Total assets: {len(df)}")
        logging.info(f"  Manifests present: {df['manifest_present'].sum()} / {len(df)}")
        logging.info(f"  Verified (INTEGRITY): {df['verified'].sum()} / {len(df)}")
        logging.info(f"  Signature valid: {df['signature_valid'].sum()} / {len(df)}")
        logging.info(f"  Hash match: {df['hash_match'].sum()} / {len(df)}")
        logging.info(f"  Trust verified: {df['trust_verified'].sum()} / {len(df)} (informational)")

        logging.info("")
        logging.info("By Asset Type:")
        for asset_type in df['asset_type'].unique():
            subset = df[df['asset_type'] == asset_type]
            logging.info(f"  {asset_type}:")
            logging.info(f"    Total: {len(subset)}")
            logging.info(f"    Verified: {subset['verified'].sum()} / {len(subset)}")

        logging.info("")
        logging.info("By Asset Source:")
        ...
        logging.info("")
        logging.info("Validation State Distribution:")
...
def main():
    """Main entry point."""
    logging.info("=" * 60)
    logging.info("C2PA Baseline Verification - Original Signed Assets")
