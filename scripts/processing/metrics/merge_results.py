...
Input CSVs:
- data/results/c2pa_validation.csv (C2PA verification results for transformed assets)
- data/results/quality_metrics.csv (Quality metrics: PSNR/SSIM/VMAF for transformed assets)
- data/results/platform_results.csv (Platform round-trip results, Phase 2.5, optional)

Output CSV:
- data/results/final_metrics.csv (Complete dataset matching CLAUDE.md schema)
...
Output:
    data/results/final_metrics.csv (~3,460 transformed + ~195 platform rows)
"""
...
logger = utils.setup_logging(log_file='data/results/logs/merge_results.log')
...
def load_c2pa_data() -> pd.DataFrame:
    ...
    if not C2PA_CSV.exists():
        logger.error(f"C2PA validation CSV not found: {C2PA_CSV}")
        sys.exit(1)

    df = pd.read_csv(C2PA_CSV)
    logger.info(f"Loaded C2PA validation data: {len(df)} rows")
...
def load_quality_data() -> pd.DataFrame:
    ...
    if not QUALITY_CSV.exists():
        logger.error(f"Quality metrics CSV not found: {QUALITY_CSV}")
        sys.exit(1)

    df = pd.read_csv(QUALITY_CSV)
    logger.info(f"Loaded transformed assets quality metrics: {len(df)} rows")
...
def load_platform_data() -> pd.DataFrame:
    ...
    if not PLATFORM_CSV.exists():
        logger.info("Platform results CSV not found - skipping platform data (Phase 2.5 not performed)")
        return pd.DataFrame()

    df = pd.read_csv(PLATFORM_CSV)
    logger.info(f"Loaded platform results data: {len(df)} rows")
...
def merge_datasets(c2pa_df: pd.DataFrame, quality_df: pd.DataFrame) -> pd.DataFrame:
    ...
    """
    logger.info("Merging C2PA validation with quality metrics...")
    ...
    if c2pa_only:
        logger.warning(f"Files in C2PA validation but not in quality metrics: {len(c2pa_only)}")
        for fname in list(c2pa_only)[:5]:
            logger.warning(f"  - {fname}")

    if quality_only:
        logger.warning(f"Files in quality metrics but not in C2PA validation: {len(quality_only)}")
...
def reorder_columns(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if missing_cols:
        logger.info(f"Columns not present in dataset: {missing_cols}")
...
def validate_dataset(df: pd.DataFrame):
    ...
    for col in df.columns:
        ...
        logger.info(f"  {col}: {dtype} ({null_count} null values)")

    # Check for duplicates
    duplicates = df[df.duplicated(subset=['filename'], keep=False)]
    if not duplicates.empty:
        logger.error(f"Found {len(duplicates)} duplicate filenames!")
    ...
    for col in int_cols:
        if col in df.columns:
            if df[col].dtype not in ['int64', 'Int64']:
                logger.warning(f"Column {col} should be integer, got {df[col].dtype}")
    ...
    if 'asset_type' in df.columns:
        logger.info("Asset type distribution:")
...
def main():
    """Main entry point."""
    logger.info("=" * 60)
    logger.info("Results Merge Script - C2PA Robustness Testing")
    ...
    if not platform_df.empty:
        logger.info("Appending platform results to dataset...")

        # Add platform-specific columns to merged_df if not present (use NaN for proper null representation)
        import numpy as np
        for col in ['platform', 'platform_mode']:
        ...
        for col in final_df.columns:
            if col not in platform_df.columns:
                if col not in ['platform', 'platform_mode']:
                    # Column should exist in platform results
                    logger.warning(f"Column '{col}' missing from platform results")

        # Append platform results
        combined_df = pd.concat([final_df, platform_df], ignore_index=True)
        logger.info(f"Combined dataset: {len(combined_df)} rows ({len(final_df)} transformed + {len(platform_df)} platform)")
    ...
    # Save final dataset (use na_rep='NA' to write NaN as "NA" instead of empty strings)
    final_df.to_csv(OUTPUT_CSV, index=False, na_rep='NA')
    logger.info(f"Final dataset saved: {OUTPUT_CSV.absolute()}")

    # Summary
    logger.info("=" * 60)
    logger.info("Merge Complete")
    if platform_df.empty:
        logger.info(f"  Input: {len(c2pa_df)} C2PA rows + {len(quality_df)} quality rows -> {len(merged_df)} merged")
    else:
        logger.info(f"  Input: {len(c2pa_df)} C2PA rows + {len(quality_df)} quality rows -> {len(merged_df)} merged + {len(platform_df)} platform")
    logger.info(f"  Output: {len(final_df)} total rows")
