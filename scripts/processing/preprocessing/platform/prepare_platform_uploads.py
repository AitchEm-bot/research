...
logger = utils.setup_logging(log_file='data/prepared_assets/platform_tests/preparation.log')
utils.log_environment_info()

# Configuration
TRANSFORMED_BASE = Path("data/prepared_assets/transformed")
PLATFORM_TESTS_BASE = Path("data/prepared_assets/platform_tests")
...
def prepare_upload(asset_path: Path, platform: str, mode: str) -> bool:
    ...
    try:
        ...
        print(f"  4. Save to: data/prepared_assets/platform_tests/{platform}/returned/")
...
def auto_sample_uploads() -> bool:
    ...
    print("\nNext steps:")
    print("  1. Review assets in data/prepared_assets/platform_tests/{platform}/uploads/")
    print("  2. Upload to respective platforms (NO filters, NO edits)")
    print("  3. Download returned files to data/prepared_assets/platform_tests/{platform}/returned/")
