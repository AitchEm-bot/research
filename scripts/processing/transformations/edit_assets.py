...
logger = utils.setup_logging(log_file='data/prepared_assets/transformed/transform_edits.log')
...
def crop_image(image_path: Path, crop_percent: int, output_dir: Path) -> Tuple[Path, bool]:
    ...
    try:
        ...
        logger.error(f"Failed to crop {image_path.name} to {crop_percent}%: {e}")
        return None, False
...
def resize_image(image_path: Path, target_size: tuple, interpolation: str, output_dir: Path) -> Tuple[Path, bool]:
    ...
    try:
        ...
        logger.error(f"Failed to resize {image_path.name} to {target_size} ({interpolation}): {e}")
        return None, False
...
def rotate_image(image_path: Path, angle: int, output_dir: Path) -> Tuple[Path, bool]:
    ...
    try:
        ...
        logger.error(f"Failed to rotate {image_path.name} by {angle}°: {e}")
        return None, False
...
def adjust_color_image(image_path: Path, adjustment_type: Literal['brightness', 'contrast', 'saturation'],
    ...
    try:
        ...
        logger.error(f"Failed to adjust {adjustment_type} for {image_path.name} by factor {factor}: {e}")
        return None, False
...
def crop_video(video_path: Path, crop_percent: int, output_dir: Path) -> Tuple[Path, bool]:
    ...
    try:
        ...
        logger.error(f"Failed to crop video {video_path.name} to {crop_percent}%: {e}")
        return None, False
...
def resize_video(video_path: Path, target_size: tuple, output_dir: Path) -> Tuple[Path, bool]:
    ...
    try:
        ...
        logger.error(f"Failed to resize video {video_path.name} to {target_size}: {e}")
        return None, False
...
def rotate_video(video_path: Path, angle: int, output_dir: Path) -> Tuple[Path, bool]:
    ...
    try:
        ...
        logger.error(f"Failed to rotate video {video_path.name} by {angle}°: {e}")
        return None, False
...
def adjust_color_video(video_path: Path, adjustment_type: Literal['brightness', 'contrast', 'saturation'],
    ...
    try:
        ...
        logger.error(f"Failed to adjust {adjustment_type} for video {video_path.name}: {e}")
        return None, False
...
def process_images(test_mode: bool = False):
    ...
    if not signed_images:
        logger.info(f"No signed images found in {IMAGES_INPUT_DIR}")
        return {'total': 0}

    if test_mode:
        signed_images = signed_images[:1]

    logger.info(f"Processing {len(signed_images)} image(s)")
    ...
    for img_path in tqdm(signed_images, desc="Processing images", unit="img"):
        ...
        for crop_pct in CROP_PERCENTAGES:
            ...
            if success:
                stats['success'] += 1
                utils.save_transformation_metadata(output_path, img_path, 'crop', {'crop_percent': crop_pct})
            else:
                stats['failed'] += 1
        ...
        for target_size in IMAGE_RESIZE_TARGETS:
            for interp in IMAGE_INTERPOLATIONS:
                ...
                if success:
                    stats['success'] += 1
                    utils.save_transformation_metadata(output_path, img_path, 'resize',
        ...
        for angle in ROTATE_ANGLES:
            ...
            if success:
                stats['success'] += 1
                utils.save_transformation_metadata(output_path, img_path, 'rotate', {'angle': angle})
            else:
                stats['failed'] += 1
        ...
        for adj_type in ['brightness', 'contrast', 'saturation']:
            for factor in COLOR_ADJUSTMENTS:
                ...
                if success:
                    stats['success'] += 1
                    utils.save_transformation_metadata(output_path, img_path, f'{adj_type}_adjustment', {'factor': factor})
                else:
                    stats['failed'] += 1
...
def process_videos(test_mode: bool = False, external_only: bool = False):
    ...
    if not signed_videos:
        logger.info(f"No signed videos found in {VIDEOS_INPUT_DIRS}")
        return {'total': 0}
    ...
    logger.info(f"Processing {len(signed_videos)} video(s)")
    ...
    for video_path in tqdm(signed_videos, desc="Processing videos", unit="vid"):
        ...
        for crop_pct in CROP_PERCENTAGES:
            ...
            if success:
                stats['success'] += 1
                utils.save_transformation_metadata(output_path, video_path, 'crop', {'crop_percent': crop_pct})
            else:
                stats['failed'] += 1
        ...
        for target_size in VIDEO_RESIZE_TARGETS:
            ...
            if success:
                stats['success'] += 1
                utils.save_transformation_metadata(output_path, video_path, 'resize', {'target_size': target_size})
            else:
                stats['failed'] += 1
        ...
        for angle in ROTATE_ANGLES:
            ...
            if success:
                stats['success'] += 1
                utils.save_transformation_metadata(output_path, video_path, 'rotate', {'angle': angle})
            else:
                stats['failed'] += 1
        ...
        for adj_type in ['brightness', 'contrast', 'saturation']:
            for factor in COLOR_ADJUSTMENTS:
                ...
                if success:
                    stats['success'] += 1
                    utils.save_transformation_metadata(output_path, video_path, f'{adj_type}_adjustment', {'factor': factor})
                else:
                    stats['failed'] += 1
...
def main():
    """Main entry point."""
    logger.info("=" * 60)
    logger.info("Asset Editing Script - C2PA Robustness Testing")
    ...
    try:
        ...
        logger.warning("ffmpeg not found - video processing will be skipped")
    ...
    if test_mode:
        logger.info("TEST MODE: Processing only one asset of each type")
    ...
    # Print summary
    logger.info("=" * 60)
    logger.info("Asset Editing Complete")
    logger.info(f"  Images: {image_stats.get('success', 0)} succeeded, {image_stats.get('failed', 0)} failed")
    logger.info(f"  Videos: {video_stats.get('success', 0)} succeeded, {video_stats.get('failed', 0)} failed")
    logger.info(f"  Total output files: {image_stats.get('success', 0) + video_stats.get('success', 0)}")
    logger.info(f"  Output base directory: {OUTPUT_BASE_DIR.absolute()}")
    logger.info(f"    Image folders:")
    logger.info(f"      Crop: 90pct/, 75pct/, 50pct/")
    logger.info(f"      Resize: bicubic/256x256/, bicubic/1024x1024/, lanczos/256x256/, lanczos/1024x1024/")
    logger.info(f"      Rotate: 90deg/, 180deg/, 270deg/")
    logger.info(f"      Color: brightness/plus20/, brightness/minus20/, contrast/plus20/, contrast/minus20/, saturation/plus20/, saturation/minus20/")
    logger.info(f"    Video folders:")
    logger.info(f"      Crop: 90pct/, 75pct/, 50pct/")
    logger.info(f"      Resize: 256x256/, 768x768/")
    logger.info(f"      Rotate: 90deg/, 180deg/, 270deg/")
    logger.info(f"      Color: brightness/plus20/, brightness/minus20/, contrast/plus20/, contrast/minus20/, saturation/plus20/, saturation/minus20/")
    logger.info("=" * 60)
